...
function ChatContainer({ messages, setMessages,scroll,setScroll,containerRef,params,friendId,setFriendId }) {
  ...
  return (
    <div
      ref={containerRef}
      className="bg-[var(--surface)] pt-[20%] pb-[calc(80px+env(safe-area-inset-bottom))]  lg:pb-7 lg:pt-5 relative h-full overflow-auto text-[var(--text)] px-5 flex flex-col gap-3"
    >
      {isFetching && <Spinner />}
      {!isFetching &&
        messages?.map((el, i) => (
